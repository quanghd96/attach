...
function attach(term, socket, token, bidirectional, buffered) {
    ...
    addonTerminal.__getMessage = function (ev) {
        var _this = this;
        var str;
        if (typeof ev.body === 'object') {
            if (!myTextDecoder) {
                myTextDecoder = new TextDecoder();
            }
            if (ev.body instanceof ArrayBuffer) {
    ...
    addonTerminal.__sendData = function (data) {
        ...
        socket.send("/app/input", {}, data);
    };
    addonTerminal._core.register(addSocketListener(socket, "/connection/client/" + token, addonTerminal.__getMessage));
    if (bidirectional) {
...
function addSocketListener(socket, type, handler) {
    socket.subscribe(type, handler);
    return {
        dispose: function () {
            if (!handler) {
                return;
            }
            socket.unSubscribe(type, handler);
            handler = null;
        }
    };
}
function detach(term, socket) {
    ...
    if (socket) {
        socket.unSubscribe("/connection/client/" + token, addonTerminal.__getMessage);
    }
